...
def flatten_nested_json_df(df: pd.DataFrame) -> pd.DataFrame:
    """
    flattens all dict-type columns (1st level) and explodes all list-type columns in a DataFrame.

    Parameters:
        df (pd.DataFrame): the input DataFrame.

    Returns:
        pd.DataFrame: transformed DataFrame with dicts flattened and lists exploded.
    """
    ...
    dict_columns = [
        ...
        if df[col].dropna().map(type).eq(dict).all()
    ]

    # detect list columns
    list_columns = [
        col for col in df.columns
        if df[col].dropna().map(type).eq(list).all()
    ]

    # flatten dict columns
    for col in dict_columns:
        flattened = pd.json_normalize(df[col].dropna()).add_prefix(f"{col}.")
