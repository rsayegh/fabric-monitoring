...
# **Class and functions: https://github.com/microsoft/semantic-link-labs/blob/main/src/sempy_labs/_authentication.py**
...
with service_principal_authentication(
    key_vault_uri = f"https://{pKeyVaultName}.vault.azure.net/",
    key_vault_tenant_id = pTenantId,
    key_vault_client_id = pFabricSpnClientId,
    key_vault_client_secret = pFabricSpnSecret
):
    # Inside here, token_provider is properly set
    token = _get_headers(token_provider.get(), audience="pbi")
...
notebookutils.notebook.exit(token) 
